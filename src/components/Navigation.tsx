...
const HomeIcon = ({ active }: { active: boolean }) => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className={`transition-all duration-300 ${active ? 'drop-shadow-[0_0_8px_rgba(168,85,247,0.8)]' : ''}`}>
    <path d="M3 12L12 3l9 9v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-7z" 
          stroke={active ? '#a855f7' : '#b78ddf'} 
          strokeWidth="1.5" 
          fill={active ? '#a855f7' : 'none'} />
    <rect x="9" y="14" width="6" height="5" rx="1" 
          fill={active ? '#fff' : '#b78ddf'} 
          opacity=".2" />
  </svg>
);
...
const ReelsIcon = ({ active }: { active: boolean }) => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className={`transition-all duration-300 ${active ? 'drop-shadow-[0_0_8px_rgba(168,85,247,0.8)]' : ''}`}>
    ...
    <rect x="8" y="9" width="8" height="6" rx="2" 
          fill={active ? '#a855f7' : '#b78ddf'} 
          opacity=".2" />
  </svg>
);

const UploadIcon = ({ active }: { active: boolean }) => (
  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" className={`transition-all duration-300 ${active ? 'drop-shadow-[0_0_12px_rgba(168,85,247,0.9)]' : ''}`}>
...
const DiscoverIcon = ({ active }: { active: boolean }) => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" className={`transition-all duration-300 ${active ? 'drop-shadow-[0_0_8px_rgba(168,85,247,0.8)]' : ''}`}>
    ...
    >
      <div className={`rounded-full flex items-center justify-center p-2 ${
        highlight 
          ...
          : active 
            ? 'bg-black/40 ring-2 ring-purple-400/60' 
            : 'bg-transparent'
      }`} 
      style={highlight ? { 
        boxShadow: '0 0 24px 8px rgba(168, 85, 247, 0.4)' 
      } : {}}>
      ...
      {badge && (
        <motion.span 
          className={`absolute -top-1 -right-2 text-xs rounded-full px-1.5 py-0.5 ${
            label === 'Inbox' ? 'bg-red-500 text-white' : 'bg-purple-400 text-white'
          } shadow font-bold`} 
          style={{ fontSize: 10 }}
...
export default function Navigation() {
  ...
  if (userType === 'creator') {
    ...
    return (
      <motion.nav
        ...
        className="fixed bottom-0 w-full flex justify-around items-center bg-surface/90 py-4 border-t border-white/10 z-50 rounded-t-2xl shadow-2xl backdrop-blur-xl"
      >
        <CreatorNavItem 
  ...
  return (
    <motion.nav
      ...
      animate={{ y: 0, opacity: 1 }}
      transition={{ type: "spring", stiffness: 100, damping: 20 }}
      className="fixed bottom-0 left-0 right-0 z-40 bg-surface/80 backdrop-blur-xl border-t border-white/10"
    >
      ...
      <div className="h-1 bg-surface/80 backdrop-blur-xl" />
    </motion.nav>
  );
} 